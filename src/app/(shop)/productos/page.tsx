...
function isValidCategoria(value: string): value is (typeof CATEGORIAS_VALIDAS)[number] {
  return (CATEGORIAS_VALIDAS as readonly string[]).includes(value);
}

function isValidMaterial(value: string): value is (typeof MATERIALES_VALIDOS)[number] {
  return (MATERIALES_VALIDOS as readonly string[]).includes(value);
}

function isValidSort(value: string): value is SortOption {
  return ['relevance', 'price_asc', 'price_desc', 'newest'].includes(value);
}
...
export default function ProductosPage() {
  ...
  const activeCategories = useMemo<string[]>(() => {
    const raw = searchParams.getAll('categoria');
    return raw.filter(isValidCategoria);
  }, [searchParams]);

  const activeMaterials = useMemo<string[]>(() => {
    const raw = searchParams.getAll('material');
    return raw.filter(isValidMaterial);
  }, [searchParams]);
              ...
              <div className="sidebar-section">
                ...
                {MATERIALES_VALIDOS.map((mat) => {
                  const count = products.filter((p) =>
                    p.variants.some((v) => v.material === mat)
                  ).length;
                  if (count === 0) return null;
